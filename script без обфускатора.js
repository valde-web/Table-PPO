document.addEventListener('DOMContentLoaded', function() {
    ...
    // Проверяем, что элементы найдены (защита от ошибок)
    ...
    const options = Array.from(itemmSelect.options); // Преобразуем HTMLCollection в Array для удобства
...
const toggleMaskButton = document.getElementById('toggleMaskButton');
    let maskVisible = false;
     ...
     function filterMaskOptions() {
        options.forEach(option => {
            const optionText = option.text.toLowerCase();
            const isMask = optionText.includes('маск');

            if (maskVisible || !isMask) {
    ...
    // Вызываем filterOptions() при загрузке страницы, чтобы применить фильтр, если в поле поиска что-то уже есть
    filterOptions();
});
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  function applyTheme(theme){
    document.documentElement.classList.remove('light','pink'); // Меняем на document.documentElement
    if(theme && theme !== 'dark'){
